...
PgSqlDatabase::PgSqlDatabase(const std::string &connectionInfo, size_t connectionPoolSize) {
//    _connection = PQconnectdb(connectionInfo.c_str());
    ...
    this->self = std::shared_ptr<PgSqlDatabase>(this, PgSqlDatabase_Deleter::DisarmedDelete<PgSqlDatabase>());
    _isConnected = true;
}

PgSqlDatabase::~PgSqlDatabase() {
//    if(this->self) {
//        auto deleter = get_deleter<PgSqlDatabase_Deleter::DisarmedDelete<PgSqlDatabase>>(this->self);
//    }
...
//    std::lock_guard<std::mutex> query_lock(_queryMutex);
    ...
    if (isError(res, connection->object->connection)) return data;
    fillDataWithRes(data, res);
    PQclear(res);
    return data;
}

RsData PgSqlDatabase::query(const std::string &sql, const std::vector<std::string> &params) {
//    std::lock_guard<std::mutex> query_lock(_queryMutex);
    LOG_DEBUG << "Query: " << sql;
    RsData data;
...
//    int paramFormats[paramsSize];
    ...
    auto connection = connections.get();
    PGresult *res = PQexecParams(connection->object->connection,
                                 sql.c_str(),
                                 static_cast<int>(paramsSize),
    ...
    if (isError(res, connection->object.get()->connection)) return data;
    fillDataWithRes(data, res);
    PQclear(res);
    return data;
}
...
std::string PgSqlDatabase::ffQuery(const std::string &sql) {
    ...
    std::string rtn;
    if (isError(res, connection->object.get()->connection)) return rtn;
...
std::string PgSqlDatabase::ffQuery(const std::string &sql, const std::vector<std::string> &params) {
    LOG_DEBUG << "Query: " << sql;
    const size_t paramsSize = params.size();
    std::vector<const char *> paramChars;
    std::vector<int> paramLengths;
    ...
    if (PQresultStatus(res) == PGRES_TUPLES_OK) {
        ...
        for (int row = 0; row < resultCount; ++row) {
            for (int col = 0; col < fieldsCount; ++col) {
                ...
                itr->second.emplace_back(PQgetvalue(res, row, col), PQgetlength(res, row, col));
            }
        }
    }
}
...
PgTable PgSqlDatabase::getTable(const std::string &schema, const std::string &tableName) {
    return PgTable(this->self, schema, tableName);
}

bool PgSqlDatabase::execute(const std::string &sql, const std::vector<std::string> &params) {
...
std::vector<PgEntity>
PgSqlDatabase::find(const PgEntity &entity,
...
PgSqlDatabase::findBy(const PgEntity &entity,
                      const std::string &sql,
                      const std::vector<std::string> &params) {
    LOG_DEBUG << "Query: " << sql;
...
void PgSqlDatabase::fillPgEntity(const PgEntity &entity, std::vector<PgEntity> &rtn, PGresult *res) {
    if (PQresultStatus(res) == PGRES_TUPLES_OK) {
        int fieldsCount = PQnfields(res);
        int resultCount = PQntuples(res);
        for (size_t row = 0; row < resultCount; ++row) {
            ...
            for (int col = 0; col < fieldsCount; ++col) {
                new_entity.at(col)->Get<PgColumnBase>().Set(PQgetvalue(res, row, col), PQgetlength(res, row, col));
...
void PgSqlDatabase::fillPgEntity(const PgEntity &entity, std::vector<std::shared_ptr<PgEntity>> &rtn, PGresult *res) {
    if (PQresultStatus(res) == PGRES_TUPLES_OK) {
        int fieldsCount = PQnfields(res);
        int resultCount = PQntuples(res);
        for (size_t row = 0; row < resultCount; ++row) {
...
void
PgSqlDatabase::copyCSV(const std::string &table, const std::string &file, std::function<void(std::string &)> callback,
    ...
    if (isError(res, connection->object->connection) || PQresultStatus(res) != PGRES_COPY_IN) {
        return;
    }
    ...
    while (sfh.HasNext() && a == 1) {
        ...
        callback(buffer);
        if (buffer.size() > 1) {
            a = PQputCopyData(connection->object->connection, buffer.c_str(), buffer.size());
        }
    }
...
void PgSqlDatabase::copyCSV(const std::string &table, const std::string &file, const std::string &delimiter) {
    StringFileHandler sfh(file);
    if (!sfh.HasNext()) {
        LOG_ERROR << "Empty file";
    }
