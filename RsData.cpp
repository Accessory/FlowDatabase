...
std::string RsData::printData() const{
    ...
    size_t row = 0;
    for (auto &pair : *this) {
        size_t cMaxLength = max_element(pair.second.begin(), pair.second.end(), lengthCompare)->size();
        cMaxLength = std::max(cMaxLength, pair.first.length());
        lenghts.emplace_back(cMaxLength + 2);
        row = std::max(pair.second.size(), row);
        rtn << std::setw(static_cast<int>(lenghts.back())) << std::left << pair.first;
    }
    rtn  << std::endl;
    for (size_t i = 0; i < row; ++i) {
        ...
        size_t pos = 0;
        for (auto &pair : *this) {
            rtn << std::setw(static_cast<int>(lenghts.at(pos++))) << std::left << pair.second.at(i).toString();
        }
    }
    auto var = rtn.str();
//   var.length();
...
size_t RsData::rowSize() const{
    ...
    return max_element((*this).begin(), (*this).end(), RsData::sizeCompare)->second.size();
}
